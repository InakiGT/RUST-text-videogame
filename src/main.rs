...
fn main() {
    ...
    let mut tag_actual : String = FIRST_TAG.to_string();

    let mut last_record: String = String::new();

    let mut datos_historia: HashMap<String, DatoHistoria> = HashMap::new();
    
    let content = fs::read_to_string(FILENAME).unwrap(); // Extrae información del archivo 
    let mut rdr = ReaderBuilder::new().delimiter(b';').from_reader(content.as_bytes()); // Trae la forma de columna del csv

    for result in rdr.records() { // Accede a cada delemento de cada fila
        let result = result.unwrap();
        let dato = DatoHistoria::new(result);

        if dato.tipo_dato == "SITUACION" {
            let record_tag = dato.tag.clone();
            datos_historia.insert(record_tag.clone(), dato); // Esto nos permitirá a un TAG en especifico
    ...
    loop {
        ...
        if let Some(data) = datos_historia.get(&tag_actual) {
            println!("{}", data.texto);
            for (i, option) in data.opciones.iter().enumerate() {
        ...
        // Si la vida es <= 0 terminar juego
        if vida <= 0 {
            println!("Has perdido");
            break;
        }
